...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  decode_operand(s, &rd, &src1, &src2, &imm, concat(TYPE_, type), &shamt); \
  __VA_ARGS__ ; \
}
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = (signed)src1 >> shamt);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , R, R(rd) = src1 >> shamt);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , R, R(rd) = src1 << shamt);
		  ...
		  IFDEF(CONFIG_FTRACE,
			  {
				...
				else if(rd == 1) trace_func_call(s->pc, s->dnpc);
				else if(rd == 0 && imm == 0) trace_func_call(s->pc, s->dnpc);
			  }
		  );
		  R(rd) = s->pc + 4; 
		  );
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(BITS(Mr(src1 + imm, 1), 8 , 0), 8 ));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu	   , I, R(rd) = Mr(src1 + imm, 2));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , S, R(rd) = (signed)src1 < (signed)src2 ? 1 : 0);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , S, R(rd) = src1 < src2 ? 1 : 0);
...
void trace_inst2ringbuf(word_t, uint32_t);

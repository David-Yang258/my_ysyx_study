module axi_uart_psram_bridge #(
    ...
    wire write_pending   = s_awvalid;
    wire write_active    = aw_full || w_full || (w_state != 3'd0);
    wire block_new_reads = write_active || write_pending;
    
    wire no_outstanding_reads = (outstanding_reads == 0);

    assign s_arready = block_new_reads ? 1'b0 : m_arready;
    assign s_awready = (!aw_full) && no_outstanding_reads;
    assign s_wready  = (!w_full); 

    wire cpu_ar_fire = s_arvalid && s_arready;
    
    // 【完美恢复】：Arbiter 修好后，我们又可以安全地依靠 s_rready 进行完美的闭环检测了，不会再死锁！
    wire cpu_r_fire  = s_rvalid && s_rready && s_rlast;

    always @(posedge clk) begin
        if (reset) begin
            outstanding_reads <= 0;
        end else begin
            if (cpu_ar_fire && !cpu_r_fire)
                outstanding_reads <= outstanding_reads + 1;
            else if (!cpu_ar_fire && cpu_r_fire)
                outstanding_reads <= outstanding_reads - 1;
        end
    end
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            W_IDLE: begin
                aw_sent <= 0; w_sent <= 0;
                if (req_ready) begin
                    if (needs_rmw) w_state <= W_RMW_AR;
                    else           w_state <= W_SEND_AW_W;
                end
            end
            ...
            W_RMW_R:  begin
                if (m_rvalid) begin
                    ...
                    if (m_rlast) w_state <= W_SEND_AW_W;
                end
            end
            W_SEND_AW_W: begin
    ...
    reg [15:0] uart_read_q;
    reg [3:0]  uart_q_wr_ptr;
    reg [3:0]  uart_q_rd_ptr;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (cpu_ar_fire) begin
                uart_read_q[uart_q_wr_ptr] <= is_uart_ar;
                uart_q_wr_ptr <= uart_q_wr_ptr + 1;
            end
            if (cpu_r_fire) begin
                uart_q_rd_ptr <= uart_q_rd_ptr + 1;
            end
        end
    end

    wire current_read_is_uart = uart_read_q[uart_q_rd_ptr];

    assign m_arvalid = rmw_active ? (w_state == W_RMW_AR) : (s_arvalid && !block_new_reads);
    ...
    // 【完美恢复】：不再强制为 1，将后端传来的 s_rready 正确发给底层，完全符合协议规范
    ...
    assign s_rdata   = current_read_is_uart ? {4{m_rdata[7:0]}} : m_rdata;
